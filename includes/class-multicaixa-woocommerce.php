<?php
/**
 * Multicaixa WooCommerce main class
 */
...
final class Multicaixa_WooCommerce {
	...
	/* Internal variables */
	public $version             = MULTICAIXA_VERSION;
	...
	private function init_hooks() {
		add_filter( 'plugin_action_links_' . plugin_basename( MULTICAIXA_PLUGIN_FILE ), array( $this, 'add_settings_link' ) );
		add_filter( 'woocommerce_payment_gateways', array( $this, 'woocommerce_add_payment_gateways' ) );
		if ( $this->wc_blocks_active ) {
		...
		add_filter( 'woocommerce_order_data_store_cpt_get_orders_query', array( $this, 'multicaixa_woocommerce_order_data_store_cpt_get_orders_query' ), 10, 3 );
	...
	public function woocommerce_add_payment_gateways_woocommerce_blocks() {
			require_once 'woocommerce-blocks/MulticaixaProxyPay.php';
			add_action(
				'woocommerce_blocks_payment_method_type_registration',
				function ( Automattic\WooCommerce\Blocks\Payments\PaymentMethodRegistry $payment_method_registry ) {
					$payment_method_registry->register( new \Automattic\WooCommerce\Blocks\Payments\Integrations\MulticaixaProxyPay() );
				}
			);
	}
	...
	public function multicaixa_order_metabox() {
		...
		add_meta_box(
			$this->multicaixa_id,
			__( 'Multicaixa payment details', 'woo-multicaixa' ),
			array( $this, 'multicaixa_order_metabox_html' ),
			$screen,
			'side',
			'core'
		);
	}
	public function multicaixa_order_metabox_html( $post_or_order_object ) {
				...
				if ( $order->get_total() < $this->min_amount ) {
					...
					if ( $order->get_total() >= $this->max_amount ) {
						...
						if (
							...
							// ProxyPay API
							$data = apply_filters(
								'multicaixa_api_create_ref_data',
								array(
							...
							} elseif ( trim( $this->get_multicaixa_setting( 'api_key' ) ) == '' ) {
	...
	public function multicaixa_create_ref( $ent, $seed, $total, $just_create_no_check = false ) {
		...
		if ( ! $just_create_no_check ) {
			...
			$orders = wc_get_orders(
				Multicaixa_WooCommerce()->maybe_translate_order_query_args(
					array(
			...
			if ( $exists ) {
				...
				$seed = rand( apply_filters( 'multicaixa_create_ref_min', 0 ), apply_filters( 'multicaixa_create_ref_max', 999999999 ) ); // For random mode - Less loop possibility
	...
	/*
	Filter to be able to use wc_get_orders with our Multicaixa references - For non-HPOS */
	// HPOS compatibility via the maybe_translate_order_query_args function, that should be called in all wc_get_orders functions
	public function multicaixa_woocommerce_order_data_store_cpt_get_orders_query( $query, $query_vars, $object, $clear_key = false ) {
		...
		foreach ( $query_vars as $key => $value ) {
			switch ( $key ) {
				...
				case '_multicaixa_express_proxypay_id': // Express by payment ID
					...
					break;
				// <
				case '_' . $this->multicaixa_id . '_end_datetime':
					// unset( $args[$key] ); //WHY?
					$query['meta_query'][] = array(
						'key'     => $key,
						'value'   => esc_attr( $value ), // WHY esc_attr?
	...
	public function admin_right_bar() {
		...
		<div id="multicaixa_rightbar">
			...
			<p>
				<h4><?php _e( 'Custom WordPress/WooCommerce development', 'woo-multicaixa' ); ?>:</h4>
				<a href="https://www.webdados.pt/contactos/<?php echo esc_attr( $this->out_link_utm ); ?>" title="<?php echo esc_attr( sprintf( __( 'Please contact %s', 'woo-multicaixa' ), 'Webdados' ) ); ?>" target="_blank">
